...
class GitVisualizer {
  ...
  visualize(graph) {
    ...
    // 獲取唯一作者列表並設置顏色比例尺
    const authors = [...new Set(this.graphData.nodes.map(node => node.author))];
  ...
  renderGraph() {
    ...
    if (this.prevGraphData) {
      this.prevGraphData.nodes.forEach(n => oldPos.set(n.id, { x: n.x, y: n.y }));
    }
    
    // 節點
    const nodeSel = this.zoomGroup
      .selectAll('.node')
      .data(nodes, d => d.id);
    ...
    const nodeEnter = nodeSel.enter()
      ...
      .attr('transform', d => {
        ...
        if (!start && d.parents && d.parents.length) start = oldPos.get(d.parents[0]);
  ...
  renderBranchLabels(oldPos) {
    ...
    labelEnter.each(function(d) {
      ...
      // 顏色設定
      let bgColor = '#10b981', textColor = '#fff';
      if (d.ref.includes('tag')) bgColor = '#f59e0b';
      else if (d.ref === 'HEAD') bgColor = '#3b82f6';
      const text = g.append('text')
        .attr('x', 0).attr('y', 0).attr('fill', textColor)
        .attr('dy', '0.35em').attr('font-size', '5px')
        .text(d.ref);
      const w = text.node().getComputedTextLength();
      g.insert('rect', 'text')
        .attr('x', -5).attr('y', -5)
        .attr('rx', 5).attr('ry', 5)
        .attr('width', w + 10).attr('height', 10)
        .attr('fill', bgColor);
    });
    // enter + update
    ...
    labels.transition().duration(800)
      .attr('transform', d => `translate(${d.node.x + 20},${d.node.y + d.index * 18})`)
      .style('opacity', 1);
  }
  ...
  showCommitDetails(commit) {
    ...
    const detailsContent = document.getElementById('commitDetailsContent');
    
    detailsContent.innerHTML = `
      <div class="commit-details">
        <div><span class="hash">提交:</span> ${commit.id}</div>
        <div><span class="author">作者:</span> ${commit.author}</div>
        <div><span class="date">日期:</span> ${commit.date}</div>
        <div><span class="message">提交訊息:</span> ${commit.message}</div>
        ${commit.refs.length > 0 ? 
          `<div><span class="refs">引用:</span> ${commit.refs.join(', ')}</div>` : ''}
        ${commit.parents.length > 0 ? 
          `<div><span class="parents">父提交:</span> ${commit.parents.join(', ')}</div>` : ''}
      </div>
    `;
  ...
  highlightCommit(commitId) {
    // 重置之前的高亮狀態
    this.zoomGroup.selectAll('.node circle')
      .attr('stroke', '#333')
      .attr('stroke-width', 1.5);
    ...
    this.zoomGroup.selectAll('.link')
      .attr('stroke', d => 
        (d.source.id === commitId || d.target.id === commitId) ? '#e11d48' : '#999'
      )
      .attr('stroke-width', d => 
    ...
    this.svg.transition()
      ...
      .call(
        ...
        d3.zoomIdentity
          ...
          .scale(finalScale) // 使用計算後的縮放比例，並居中對齊
      );
  }
