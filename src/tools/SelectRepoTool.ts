...
export class SelectRepoTool implements vscode.LanguageModelTool<ISetRepoPath> {
    ...
    async invoke(options: vscode.LanguageModelToolInvocationOptions<ISetRepoPath>) {
        ...
        if (matchedRepo) {
            ...
            return new vscode.LanguageModelToolResult([
                new vscode.LanguageModelTextPart(`Git repository set to: ${matchedRepo.label}`)
            ]);
        } else {
            return new vscode.LanguageModelToolResult([
                new vscode.LanguageModelTextPart(`Invalid repository path: ${inputPath}`)
            ]);
        }
    }
}
